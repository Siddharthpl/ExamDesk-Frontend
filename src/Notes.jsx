...
function Notes() {
  ...
  useEffect(() => {
    const loadOption = async () => {
      const pyqServerData = await fetch(
      ...
      if (optionValue == "subject") {
        ...
        const filteredPyqSubjectOptionData = pyqSubjectOptionData.filter(
          (val, index) => pyqSubjectOptionData.indexOf(val) == index
        );
      ...
      if (optionValue === "chapter") {
        ...
        const filteredPyqFacultyOptionData = pyqFacultyOptionData.filter(
          (val, index) => pyqFacultyOptionData.indexOf(val) == index
        );
  ...
  const search = async () => {
    ...
    // console.log(`https://examdesk-backend.onrender.com/api/allNote?${optionValue}=${searchBarValue} is searched`);
    const searchedData = await fetch(
      `https://examdesk-backend.onrender.com/api/allNote?${optionValue}=${searchBarValue}`
    );
              ...
              suggestion.map((suggestion, i) => {
                return (
                  <div
                    ...
                    className="note-search-suggestion"
                  >
                    {suggestion}
                  </div>
                );
                  ...
                  <div key={i} className="note-card">
                    ...
                    <div className="note-card-content">
                      <div className="note-card-content-1">
                        <img
                          src="https://i.postimg.cc/fbvjMqxy/image-removebg-preview-4.png"
                          alt=""
                          srcSet=""
                        />
                      </div>
                      <div className="note-card-content-2">
                        ...
                        <a
                          target="_blank"
                          href={docUrl + searchResult.filePath + docMethod}
                        >
