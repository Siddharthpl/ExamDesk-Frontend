...
function PyqUpload() {
  ...
  useEffect(() => {
    const loadSubject = async () => {
      const subjectNoteFetch = await fetch(
        "https://examdesk-backend.onrender.com/api/allPyq"
      );
      ...
      const filteredSuggestion = myPyqSubjectData.filter(
        (val, index) => myPyqSubjectData.indexOf(val) == index
      );
  ...
  const pyqUpload = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://examdesk-backend.onrender.com/api/pyq/upload/",
        formdata,
        pyqUploadOptions
      );
      ...
      <div className="upload">
        <div className="upload-menu">
          <NavLink to="/upload/notes">
            <button
              onClick={notesButtonON}
              style={{
                background: "rgb(173, 160, 160)",
                color: "black",
              }}
              className="upload-menu-notes"
            >
              Notes
            </button>
          </NavLink>

          <NavLink to="/upload/pyq">
            <button onClick={pyqButtonON} className="upload-menu-pyq">
              Pyq
            </button>
          </NavLink>
        </div>
        ...
        <div className="upload-pyq">
          <form onSubmit={(e) => pyqUpload(e)} className="pyq-upload-form">
            <label className="select-notes-container" htmlFor="Select Notes">
              <img
                src="https://i.postimg.cc/fLbNF8mc/ezgif-com-gif-maker-3.gif"
                alt=""
                srcSet=""
              />
              ...
              <div className="select-pyq-button">
                <input
                  ...
                  onChange={(e) => notesFile(e)}
                />
              </div>
            </label>

            <input
              className="pyq-text-input"
              value={credit}
            ...
            <div className="pyq-submit">
              <button type="submit" name="Upload" id="">
                Upload
              </button>
            </div>

            <CircularProgressbar
              className="pyq-circular-progress"
              value={noteProgress}
              text={`${noteProgress}%`}
              styles={buildStyles({
                rotation: 0.25,
