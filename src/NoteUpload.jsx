  ...
  useEffect(() => {
    const loadSubject = async () => {
      const subjectNoteFetch = await fetch(
        "https://examdesk-backend.onrender.com/api/allNote"
      );
      const subjectNoteData = await subjectNoteFetch.json();
      let myNoteSubjectData = subjectNoteData.map((obj) => {
        return obj.subject;
      });
      const filteredSuggestion = myNoteSubjectData.filter(
        (val, index) => myNoteSubjectData.indexOf(val) == index
      );
  ...
  const notesUpload = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://examdesk-backend.onrender.com/api/note/upload/",
        formdata,
        noteUploadOptions
      );
      setNotesMsg("File Upload successfully");
      ...
      <div className="upload">
        <div className="upload-menu">
          <NavLink to="/upload/notes">
            <button onClick={notesButtonON} className="upload-menu-notes">
              Notes
            </button>
          </NavLink>
        ...
        <div className="upload">
          <div className="upload-notes">
            <form
              onSubmit={(e) => notesUpload(e)}
              className="notes-upload-form"
            >
              ...
              <CircularProgressbar
                className="note-circular-progress"
                value={noteProgress}
                text={`${noteProgress}%`}
                styles={buildStyles({
                  rotation: 0.25,
